...
export function buildPirateCove(B, arena = false) {
  ...
  // === MACRO STRUCTURE: Galleon Sterncastle at (-25, 0, 5) ===
  box(-25, 0, 5, 8.0, 2.0, 6.0, { ink: BL });
  box(-25, 0 + 2.0, 5, 7.5, 2.5, 5.5, { ink: OR });
  slab(-25 - 4.0, 5 - 3.2, -25 + 4.0, 5 + 3.2, 0 + 4.5, 0.3, { ink: OR });
  rail(-25 - 4.0, 5 - 3.2, -25 + 4.0, 5 - 3.2, 0 + 4.5, { ink: BK });
  rail(-25 - 4.0, 5 + 3.2, -25 + 4.0, 5 + 3.2, 0 + 4.5, { ink: BK });
  rail(-25 - 4.0, 5 - 3.2, -25 - 4.0, 5 + 3.2, 0 + 4.5, { ink: BK });
  box(-25, 0 + 4.8, 5, 1.0, 3.0, 0.5, { ink: BK });
  ring(-25, 0 + 9.3, 5, 'z');
  pickup(-25, 0 + 4.7, 5);
  ...
  // Prop: Cargo Crate
  box(-19, 0, -3, 1.8, 1.8, 1.8, { ink: BL });
  box(-19, 0, -3, 1.9, 1.9, 1.9, { noCollide: true, ink: BK });

  // Prop: Mooring Bollard
  cyl(-31, 0, -23, 0.4, 1.0, { seg: 8, ink: BK });
  cyl(-31, 0 + 1.0, -23, 0.5, 0.2, { seg: 8, ink: BK });

  // Prop: Cargo Crate
  box(-31, 0, -27, 1.8, 1.8, 1.8, { ink: BL });
  box(-31, 0, -27, 1.9, 1.9, 1.9, { noCollide: true, ink: BK });
  ...
  // Prop: Timber Stack
  box(17, 0, -11, 3.0, 1.0, 1.2, { ink: OR });

  // Prop: Timber Stack
  box(49, 0, -39, 3.0, 1.0, 1.2, { ink: OR });

  // Prop: Mooring Bollard
  cyl(45, 0, -15, 0.4, 1.0, { seg: 8, ink: BK });
  cyl(45, 0 + 1.0, -15, 0.5, 0.2, { seg: 8, ink: BK });
  ...
  // Prop: Timber Stack
  box(49, 0, -23, 3.0, 1.0, 1.2, { ink: OR });
  ...
  // Prop: Timber Stack
  box(-39, 0, 45, 3.0, 1.0, 1.2, { ink: OR });

  // Prop: Timber Stack
  box(-35, 0, 29, 3.0, 1.0, 1.2, { ink: OR });

  // Prop: Mooring Bollard
  cyl(-43, 0, 29, 0.4, 1.0, { seg: 8, ink: BK });
  cyl(-43, 0 + 1.0, 29, 0.5, 0.2, { seg: 8, ink: BK });
  ...
  // Prop: Timber Stack
  box(25, 0, 37, 3.0, 1.0, 1.2, { ink: OR });
  ...
  for (let px = -40; px <= 40; px += 15) {
    for (let pz = -40; pz <= 40; pz += 15) {
      if (Math.abs(px) > 15 || Math.abs(pz) > 15) { // Avoid central dais
        box(px, 0, pz, 1.8, 1.8, 1.8, { ink: BL });
        cyl(px + 3, 0, pz, 0.4, 1.0, { seg: 8, ink: BK });
      }
    }
  }
