# MAP CONCEPT 04: ZEN

## 1. Spatial Coordinates & Level Envelope
- **Coordinate Boundary**: X: [-55.0m, +55.0m], Z: [-55.0m, +55.0m], Y: [0.0m, 18.0m] (Solo) / [0.0m, 30.0m] (Arena).
- **Perimeter Thickness**: 6.0m solid outer bounding hull.
- **Vertical Tiers**:
  - Tier 1 (Ground Floor / Foundation): Y = 0.0m
  - Tier 2 (Intermediate Balconies & Terraces): Y = 4.5m
  - Tier 3 (Apex Catwalks & Overlooks): Y = 9.0m

## 2. Aesthetic & Ink Material System
- **Environment Dossier**: ZEN - ZEN THEMATIC SECTOR
- **Tactical Category**: `zen`
- **Engagement Profile**: CQB Corridors & Apex Catwalk Grapple Flanking
- **Primary Ink**: INK.BLUE (Structural concrete, foundational slabs, perimeter enclosure)
- **Secondary Ink**: INK.BLACK (Iron frames, safety railings, mechanical linework)
- **Accent Inks**: INK.ORANGE & INK.RED (High-hazard zones, grapple anchors, vantage markers)

## 3. Perimeter Enclosure & Gateways
- 4 Cardinal reinforced exterior walls (Thickness = 6.0m, Height = 18.0m / 30.0m).
- 4 Cardinal sliding doorframes with 2.8m clear vertical clearance and 1.8m width.
- Perimeter elevated walkways at Y = 5.5m and Y = 9.0m for long-distance rifle coverage.

## 4. Sector 1 (North-West)
- High-density tactical cluster with fortified corner shelter and crouch cover barricades.
- **Thematic Micro Props (Tier 1)**: toro stone lantern (0.8m x 1.4m x 0.8m), engraved kanji marker slab.
- **Thematic Meso Props (Tier 2)**: bamboo shishi-odoshi water fountain, dressing crates.
- Low crouch cover and chest cover nodes (1.1m height) for waist-high bullet defilade and vaulting.

## 5. Sector 2 (North-East)
- Intersecting ramps and multi-level CQB alleyway with intermediate dressing clutter.
- **Thematic Micro Props (Tier 1)**: bamboo chōzubachi water basin, incense burner bowl.
- **Thematic Meso Props (Tier 2)**: wooden tatami bench (2.2m x 0.6m x 0.8m).
- Overhead grapple ring at Y = 11.5m for rapid vertical ingress.

## 6. Sector 3 (South-West)
- Flanking corridor with stepped parapets, berm barriers, and cover partitions.
- **Thematic Micro Props (Tier 1)**: toro stone lantern (0.8m x 1.4m x 0.8m), incense burner bowl.
- Sightlines directed toward Central Tier 2 Dais.

## 7. Sector 4 (South-East)
- Stepped vantage outpost with elevated sniper nesting perch and vault barricade.
- **Thematic Meso Props (Tier 2)**: slatted shoji privacy divider.
- Anti-camp open rear vector preventing entrenched camping.

## 8. Central Sector (Plaza & Apex Catwalk)
- Central Tier 2 Dais (Y = 4.5m) connected via dual 14-step stairways.
- **Thematic Macro Prop (Tier 3)**: multi-tiered sorin pagoda apex & vermilion arched taiko bridge & earthen kura storehouse wall & slotted bell tower pavilion.
- Apex bridge (Y = 9.0m) overlooking all 4 quadrant lanes with full 360-degree grapple sightlines.

## 9. Overhead & Aerial Traversals
- 7 Grapple rings positioned at safe distances (>= 1.5m) from structural colliders.
- **Kinetic Elements (Tier 4)**: flowing liquid ink stream, paper cranes spiraling on thermal updrafts, swinging chime pendulum.
- Dynamic paper planes circling at Y = 22.0m for aerial hitching.

## 10. Stairway Mathematics & Headroom Clearances
- **Step Rise**: 0.2857m (14 steps per 4.0m elevation rise).
- **Step Run**: 0.45m.
- **Intermediate Rest Landings**: Slabs provided at Y = 4.5m and Y = 9.0m to break long vertical runs.
- **Required Headroom**: >= 2.0m continuous vertical clearance guaranteed.
- **Aperture Cutout**: Floor slabs above stair entries maintain full clearance without ceiling collisions.

## 11. Variations Matrix (Solo vs. Arena Match)
- **Solo**: Focused urban block, contained sky lid, wave spawner distribution.
- **Arena**: Expanded P = 68.0m perimeter, 9 balanced team spawn points, dome ribbing.

## 12. Spawn Points & Vantage Snipers
- 8 Solo wave spawns, 2 Team bases (5 spawns each).
- 4 Elevated sniper nests with anti-camp open rear vectors.
- 6 Balanced ammo and health pickup nodes.

## 13. Level Designer Checklist
- [x] All stairways maintain >= 2.0m vertical headroom.
- [x] Minimum 150 colliders registered for dense tactical geometry.
- [x] Grapple rings maintain >= 1.5m wall clearance.
- [x] Zero dead-end pinch points (< 1.8m width).


## 14. Interactive Macro-Structures (Tier 3 & 4)
- **Bonshō Bell & Tea Pavilion (X=-43, Z=-43)**: Multi-tiered wooden open shrine with playable intermediate stairs, upper meditation deck, hanging bronze bell, and apex grapple beam.
- **Engawa Lookout Sanctuary (X=-43, Z=29)**: Multi-room veranda teahouse with tatami platforms, waist-high railing cover, and overlooking sniper terrace.


## 14. Interactive Macro-Structures (Tier 3 & 4)
- **Bonshō Bell & Tea Pavilion (X=-27, Z=-43)**: Multi-tiered wooden open shrine with playable intermediate stairs, upper meditation deck, hanging bronze bell, and apex grapple beam.
- **Engawa Lookout Sanctuary (X=-27, Z=29)**: Multi-room veranda teahouse with tatami platforms, waist-high railing cover, and overlooking sniper terrace.


## 14. Interactive Macro-Structures (Tier 3 & 4)
- **Bonshō Bell & Tea Pavilion (X=-43, Z=-43)**: Multi-tiered wooden open shrine with playable intermediate stairs, upper meditation deck, hanging bronze bell, and apex grapple beam.
- **Engawa Lookout Sanctuary (X=-43, Z=29)**: Multi-room veranda teahouse with tatami platforms, waist-high railing cover, and overlooking sniper terrace.


## 14. Interactive Macro-Structures (Tier 3 & 4)
- **Bonshō Bell & Tea Pavilion (X=-27, Z=-43)**: Multi-tiered wooden open shrine with playable intermediate stairs, upper meditation deck, hanging bronze bell, and apex grapple beam.
- **Engawa Lookout Sanctuary (X=-27, Z=29)**: Multi-room veranda teahouse with tatami platforms, waist-high railing cover, and overlooking sniper terrace.


## 14. Interactive Macro-Structures (Tier 3 & 4)
- **Bonshō Bell & Tea Pavilion (X=-43, Z=-43)**: Multi-tiered wooden open shrine with playable intermediate stairs, upper meditation deck, hanging bronze bell, and apex grapple beam.
- **Engawa Lookout Sanctuary (X=-43, Z=29)**: Multi-room veranda teahouse with tatami platforms, waist-high railing cover, and overlooking sniper terrace.


## 14. Interactive Macro-Structures (Tier 3 & 4)
- **Bonshō Bell & Tea Pavilion (X=-27, Z=-43)**: Multi-tiered wooden open shrine with playable intermediate stairs, upper meditation deck, hanging bronze bell, and apex grapple beam.
- **Engawa Lookout Sanctuary (X=-27, Z=29)**: Multi-room veranda teahouse with tatami platforms, waist-high railing cover, and overlooking sniper terrace.


## 14. Interactive Macro-Structures (Tier 3 & 4)
- **Bonshō Bell & Tea Pavilion (X=-43, Z=-43)**: Multi-tiered wooden open shrine with playable intermediate stairs, upper meditation deck, hanging bronze bell, and apex grapple beam.
- **Engawa Lookout Sanctuary (X=-43, Z=29)**: Multi-room veranda teahouse with tatami platforms, waist-high railing cover, and overlooking sniper terrace.


## 14. Interactive Macro-Structures (Tier 3 & 4)
- **Bonshō Bell & Tea Pavilion (X=-43, Z=-31)**: Multi-tiered wooden open shrine with walkable mezzanine, bronze bell, and apex grapple beam.
- **Engawa Lookout Sanctuary (X=-43, Z=41)**: Multi-room veranda teahouse with tatami platforms, waist-high railing cover, and sniper terrace.
- **Torii Gate Overlook Platform (X=-31, Z=-43)**: Vermilion torii gate with elevated walkway beam and flanking stone lanterns.
- **Bonshō Bell & Tea Pavilion (X=-31, Z=29)**: Multi-tiered wooden open shrine with walkable mezzanine, bronze bell, and apex grapple beam.


## 14. Interactive Macro-Structures (Tier 3 & 4)
- **Bonshō Bell & Tea Pavilion (X=-31, Z=-31)**: Multi-tiered wooden open shrine with walkable mezzanine, bronze bell, and apex grapple beam.
- **Torii Gate Overlook Platform (X=-13, Z=-37)**: Vermilion torii gate with elevated walkway beam and flanking stone lanterns.
- **Torii Gate Overlook Platform (X=-13, Z=-19)**: Vermilion torii gate with elevated walkway beam and flanking stone lanterns.
- **Bonshō Bell & Tea Pavilion (X=-13, Z=41)**: Multi-tiered wooden open shrine with walkable mezzanine, bronze bell, and apex grapple beam.


## 14. Interactive Macro-Structures (Tier 3 & 4)
- **Bonshō Bell & Tea Pavilion (X=-43, Z=-31)**: Multi-tiered wooden open shrine with walkable mezzanine, bronze bell, and apex grapple beam.
- **Engawa Lookout Sanctuary (X=-43, Z=29)**: Multi-room veranda teahouse with tatami platforms, waist-high railing cover, and sniper terrace.
- **Torii Gate Overlook Platform (X=-31, Z=-43)**: Vermilion torii gate with elevated walkway beam and flanking stone lanterns.
- **Torii Gate Overlook Platform (X=-13, Z=17)**: Vermilion torii gate with elevated walkway beam and flanking stone lanterns.
