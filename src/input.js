...
export class Input {
  constructor(canvas) {
    ...
    this.aimSensMultiplier = 0.85;
    this.stickyAim = true; // Tactical style Sticky Aim / Toggle ADS
    this.stickyAimActive = false;
    ...
    document.addEventListener('mousedown', (e) => {
      ...
      if (a === 'aim' && this.stickyAim) {
        this.stickyAimActive = !this.stickyAimActive;
        if (this.mobile) {
          if (this.stickyAimActive) this.mobile.setAimLocked(true);
          else this.mobile.cancelAim();
        }
      } else if (a) {
    ...
    document.addEventListener('mouseup', (e) => {
      const a = MOUSEMAP[e.button];
      if (a === 'aim' && this.stickyAim) return; // Keep sticky aim active on button release
  ...
  update(dt) {
    ...
    if (this.mobile) {
      const mv = this.mobile.getMove();
      this.touchMove.x = mv.x;
      this.touchMove.y = mv.y;
      const lk = this.mobile.getLook();
      this.touchLook.x = lk.x;
      this.touchLook.y = lk.y;
      // Buttons
      this.touchPrev = { ...this.touchButtons };
      this.touchButtons = {};
      const actions = ['fire','aim','jump','crouch','reload','grapple','grenade','melee','sprint','dash','slot1','slot2','slot3','slot4','slot5'];
      for (const a of actions) {
        if (this.mobile.getButton(a)) {
          this.touchButtons[a] = true;
          s[a] = true;
        }
      }
      // Tactical 1-Tap ADS Fire: triggers both ADS aim and continuous fire
      if (this.mobile.getButton('adsfire')) {
        this.touchButtons['fire'] = true;
        this.touchButtons['aim'] = true;
        s.fire = true;
        s.aim = true;
      }
      if (this.mobile.getButton('hipfire')) {
        this.touchButtons['fire'] = true;
        s.fire = true;
      }
      // Any touch counts as active
      if (mv.x !== 0 || mv.y !== 0 || lk.x !== 0 || lk.y !== 0 || Object.keys(this.touchButtons).length > 0) {
        this.lastActive = performance.now();
        this.anyInput = true;
      }
    }

    // Sticky Aim / Toggle ADS state enforcement (Mobile & Desktop)
    if (this.stickyAimActive || (this.mobile && this.mobile.isAimLocked)) {
      s.aim = true;
      this.touchButtons['aim'] = true;
    }
    // Auto-cancel sticky ADS on active sprint
    if (s.sprint) {
      if (this.stickyAimActive) this.stickyAimActive = false;
      if (this.mobile && this.mobile.isAimLocked) this.mobile.cancelAim();
    }

    // movement from keys + touch
    let mx = (s.right ? 1 : 0) - (s.left ? 1 : 0);
    let my = (s.forward ? 1 : 0) - (s.back ? 1 : 0);
    // Add touch joystick with priority (fluid tactical feel)
    if (this.touchMove.x !== 0 || this.touchMove.y !== 0) {
      mx = this.touchMove.x;
      my = this.touchMove.y;
    }
    // look from mouse + touch with ADS / Aim sensitivity multiplier
    const isAiming = !!(s.aim || s.adsfire || this.mouseBtns.aim);
    const aimMult = isAiming ? (this.aimSensMultiplier || 0.85) : 1.0;
    let lx = -this.mx * this.mouseSens * aimMult, ly = -this.my * this.mouseSens * aimMult; this.mx = 0; this.my = 0;
    if (this.touchLook.x !== 0 || this.touchLook.y !== 0) {
      lx += this.touchLook.x * aimMult;
      ly += this.touchLook.y * aimMult;
    }
  ...
  down(a) {
    if (a === 'aim' && (this.stickyAimActive || (this.mobile && this.mobile.isAimLocked))) return true;
    return !!this.state[a];
  }
  get idleSeconds() { return (performance.now() - this.lastActive) / 1000; }
  pressed(a) { 
    const touchPressed = !!this.touchButtons[a] && !this.touchPrev[a];
    return (!!this.state[a] && !this.prev[a]) || (!!this.padState[a] && !this.padPrev[a]) || touchPressed; 
  }
