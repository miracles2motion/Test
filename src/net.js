...
export class Net {
  ...
  async quickJoin(meta = {}, onStatus = null) {
    ...
    if (onStatus) onStatus('looking for open lobbies...');
    ...
    const winner = await new Promise((resolve) => {
      ...
      for (const hostId of ids) {
        ...
        conn.on('data', (msg) => { if (!msg || a.done) return; if (msg.t === 'welcome') { a.done = true; pending--; offers.push({ conn, hostId, welcome: msg.d }); if (onStatus) onStatus(`found ${offers.length} open lobbies...`); if (pending <= 0) settle(); else if (!gather) gather = setTimeout(settle, 1500); } else if (msg.t === 'refused') failOne(a); });
