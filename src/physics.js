...
export class World {
  ...
  _resolveAxis(body, axis, move) {
    const ax = AX[axis]; const p = body.pos; let pushed = 0; const dir = Math.sign(move);
    const lim = Math.abs(move) > 1e-9 ? Math.abs(move) + 0.05 : 0.02;
    for (let iter = 0; iter < 4; iter++) {
      ...
      for (let i = 0; i < _out.length; i++) {
        ...
        if (dir > 0 && -cNeg <= lim) corr = cNeg;
          ...
          const m = Math.abs(cNeg) < Math.abs(cPos) ? cNeg : cPos;
          if (Math.abs(m) <= lim) corr = m;
        }
