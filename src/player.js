...
const _v = new THREE.Vector3(), _v2 = new THREE.Vector3(), _v3 = new THREE.Vector3(), _d = new THREE.Vector3(), _fwd = new THREE.Vector3(), _right = new THREE.Vector3(), _down = new THREE.Vector3(0, -1, 0), _aimV = new THREE.Vector3();
...
export class Player {
  ...
  switchTo(i, silent = false) {
    ...
    if (this.ctx?.input?.mobile?.isAimLocked) this.ctx.input.mobile.cancelAim();
    if (this.ctx?.input?.stickyAimActive) this.ctx.input.stickyAimActive = false;
  ...
  idleCam(t, dt = 0.016) {
    const c = this.camera;
    if (this.ctx.level && this.ctx.level.key === 'studio') {
      // Studio showcase view: close up on player, placed on right side of screen
      c.position.set(-6, 3, 16);
      c.lookAt(0, 3, 0);
      this.rig.visible = true;
      this.yaw = -Math.PI / 8; // Turn player slightly to the left
      this.pitch = -0.05; // Look slightly down
      // Add subtle breathing movement
      const breath = Math.sin(t * 2) * 0.05;
      this.eyeH = 4.0 + breath;
      if (this.weapon && this.weapon.animate) {
  ...
  update(dt) {
    ...
    if (b.pos.y < -12 || Math.abs(b.pos.x) > 95 || Math.abs(b.pos.z) > 95) {
      this.detachGrapple(false); b.pos.copy(ctx.level.playerStart); b.vel.set(0, 0, 0); if (this.ctx.game.mode !== 'explore') { this.takeDamage(20, null); if (this.onFall) this.onFall(); }
