...
export const INK_COLORS = [
  new THREE.Vector3(0.14, 0.28, 0.76), // blue ballpoint (crisp, authentic biro)
  new THREE.Vector3(0.86, 0.14, 0.22), // red pen
  new THREE.Vector3(0.22, 0.24, 0.30), // graphite
...
void main() {
  ...
  vec2 wob = vec2(0.0);
  if (uQuality > 0.5) {
    wob = vec2(vnoise(nuv * 6.0 + 11.3), vnoise(nuv * 6.0 + 37.0)) - 0.5;
  }
  ...
  if (!sky) {
    if (shade < 0.0) hatch = 0.82;
      ...
      if (d < 2.0) {
        ...
        sp = 0.16 * lod; w = sp * 0.125;
        if (uQuality > 1.5) {
          hp += (vnoise(hp * (2.5 / sp)) - 0.5) * sp * 0.4; // hand-drawn waver on high/extreme
        }
      }
      ...
      float h3 = (uQuality > 0.5) ? stripes(hp, d3, sp * 0.7, w) : 0.0;
      hatch = h1 * smoothstep(0.60, 0.46, shade);
      hatch = max(hatch, h2 * smoothstep(0.40, 0.28, shade));
      if (uQuality > 0.5) hatch = max(hatch, h3 * smoothstep(0.22, 0.10, shade));
      hatch = max(hatch, smoothstep(0.10, 0.0, shade) * 0.28);
    }
  }
  ...
  float grain = 0.0;
  if (uQuality > 0.5) {
    grain = vnoise(pp * 0.8) * 0.6 + (uQuality > 1.5 ? vnoise(pp * 0.17) * 0.4 : 0.0);
  }
...
export class InkRenderer {
  constructor(canvas) {
    ...
    const savedQuality = (typeof localStorage !== 'undefined' && localStorage.getItem('doodle_graphics_quality')) || 'high';
    const savedResScale = (typeof localStorage !== 'undefined' && Number(localStorage.getItem('doodle_res_scale'))) || 100;
    this.setGraphicsQuality(savedQuality, savedResScale);
    window.addEventListener('resize', () => this.resize());
  }
  setGraphicsQuality(preset = 'high', resScale = 100) {
    this.qualityPreset = preset;
    this.resScalePercent = resScale;
    const deviceDpi = (typeof window !== 'undefined' && window.devicePixelRatio) || 1;
    let maxDpi = 1.8;
    ...
    const targetDpi = Math.min(deviceDpi * baseMultiplier * (resScale / 100), maxDpi);
    this.pixelRatio = Math.max(0.5, Math.min(3.5, targetDpi));
    if (this.post && this.post.uniforms) {
      if (this.post.uniforms.uExtreme) this.post.uniforms.uExtreme.value = (preset === 'extreme') ? 1.0 : 0.0;
      if (this.post.uniforms.uQuality) this.post.uniforms.uQuality.value = qVal;
    }
    this.resize();
    return { pixelRatio: this.pixelRatio, rw: Math.floor(window.innerWidth * this.pixelRatio), rh: Math.floor(window.innerHeight * this.pixelRatio) };
  }
  resize() {
  ...
  render(time, fx = {}) {
    ...
    if (this._pendingScreenshot) {
      const cb = typeof this._pendingScreenshot === 'function' ? this._pendingScreenshot : null;
      this._pendingScreenshot = false;
      try {
        const dataUrl = r.domElement.toDataURL('image/png');
        const a = document.createElement('a');
        const now = new Date();
        const stamp = now.toISOString().replace(/[:.]/g, '-').slice(0, 19);
        a.download = `doodle-strike-${stamp}.png`;
        a.href = dataUrl;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        if (cb) cb(null, dataUrl);
        if (typeof window !== 'undefined' && window.__onScreenshotSaved) {
          window.__onScreenshotSaved(dataUrl);
        }
      } catch (err) {
  ...
  takeScreenshot(cb = null) {
    this._pendingScreenshot = cb || true;
  }
}
