...
class ViewModel {
  ...
  animate(dt, st) {
    ...
    this.swayPos.x = damp(this.swayPos.x, lx * 0.5 * ia, 10, dt); this.swayPos.y = damp(this.swayPos.y, ly * 0.35 * ia, 10, dt);
    this.swayRot.x = damp(this.swayRot.x, ly * ia, 10, dt); this.swayRot.y = damp(this.swayRot.y, -lx * ia, 10, dt); 
    this.swayRot.z = damp(this.swayRot.z, (-lx * 1.8 - st.strafe * 0.06) * ia, 8, dt);
    const bobX = Math.sin(st.bobPhase) * 0.013 * st.bobAmt * ia, bobY = Math.abs(Math.cos(st.bobPhase)) * 0.013 * st.bobAmt * ia;
    ...
    p.x += this.swayPos.x + bobX + rk.x * ia + this.sprintAmt * 0.06;
    p.y += this.swayPos.y + bobY + rk.y * ia - eq * 0.32 - st.landDip * 0.35 * ia - this.sprintAmt * 0.09;
    p.z += rk.z + this.sprintAmt * 0.05;
    r.set(
      this.baseRot.x * ia + this.swayRot.x + rr.x * ia - eq * 0.9 + this.sprintAmt * 0.4 + st.landDip * 0.5 * ia,
      this.baseRot.y * ia + this.swayRot.y + rr.y * ia - this.sprintAmt * 0.55,
      this.baseRot.z * ia + this.swayRot.z + rr.z * ia + this.sprintAmt * 0.18 + st.slideTilt * 0.4 * ia
    );
...
export class Rifle extends Gun {
  ...
  build() {
    ...
    // Top picatinny rail
    bx(0.095, 0.015, 0.8, 0, 0.065, -0.15, dark, g);
    for(let i=0; i<15; i++) bx(0.1, 0.005, 0.01, 0, 0.075, -0.5 + i*0.05, dark, g);

    // Magazine with ribs
    this.magMesh = new THREE.Group(); this.magMesh.position.set(0, -0.16, -0.06); this.magMesh.rotation.x = 0.15; g.add(this.magMesh);
    bx(0.06, 0.2, 0.1, 0, 0, 0, mat, this.magMesh); 
    for(let i=0; i<4; i++) bx(0.065, 0.01, 0.08, 0, -0.06 + i*0.03, 0, dark, this.magMesh);
    this.magY = -0.16;

    // Tactical stock (hollowed out) and pistol grip
    bx(0.07, 0.03, 0.3, 0, 0.03, 0.4, mat, g); // Top tube
    bx(0.04, 0.03, 0.24, 0, -0.03, 0.4, mat, g); // Bottom tube
    bx(0.07, 0.11, 0.04, 0, -0.01, 0.53, dark, g); // Buttpad
    bx(0.04, 0.08, 0.04, 0, -0.01, 0.27, mat, g); // Stock joint
    const grip = bx(0.05, 0.14, 0.06, 0, -0.13, 0.12, mat, g); grip.rotation.x = 0.3;

    // Holographic sight
    bx(0.06, 0.04, 0.12, 0, 0.09, -0.05, dark, g); // Base
    frame(0.08, 0.07, 0.012, 0.04, 0, 0.135, -0.05, mat, g); // Window frame
    sph(0.0015, 0, 0.135, -0.05, this.red, g, 5); // Red dot
    // Eotech style outer shroud
    frame(0.09, 0.08, 0.006, 0.08, 0, 0.14, -0.05, dark, g);
...
export class Shotgun extends Gun {
  ...
  build() {
    const g = this.root, mat = this.mat, dark = this.dark;
    // Main receiver and barrels
    bx(0.09, 0.13, 0.42, 0, 0, 0.05, mat, g); 
    cyl(0.021, 0.92, 0, 0.05, -0.62, dark, g); // Upper barrel
    cyl(0.019, 0.72, 0, -0.02, -0.5, dark, g); // Lower mag tube

    // Ejection port
    bx(0.01, 0.04, 0.12, 0.046, 0.02, 0.05, dark, g);

    // Heat shield over the barrel
    for(let i=0; i<8; i++) frame(0.05, 0.05, 0.005, 0.04, 0, 0.05, -0.3 - (i*0.08), mat, g);

    // Side saddle shell holder
    bx(0.015, 0.06, 0.18, -0.046, 0, 0.05, dark, g);
    for(let i=0; i<4; i++) cyl(0.012, 0.05, -0.055, 0, 0.11 - (i*0.04), this.red, g, 'x');

    // Ribbed pump forend
    this.foreEnd = new THREE.Group(); this.foreEnd.position.set(0, 0.01, -0.46); g.add(this.foreEnd); this.foreEndZ = -0.46;
    bx(0.078, 0.085, 0.27, 0, 0, 0, mat, this.foreEnd);
    for(let i=0; i<6; i++) bx(0.082, 0.09, 0.01, 0, 0, -0.1 + (i*0.04), dark, this.foreEnd);

    // Stock and grip
    const stock = bx(0.07, 0.12, 0.34, 0, -0.04, 0.42, mat, g); stock.rotation.x = 0.08; 
    const grip = bx(0.05, 0.13, 0.06, 0, -0.13, 0.16, mat, g); grip.rotation.x = 0.35;
...
export class Sniper extends Gun {
  ...
  build() {
    const g = this.root, mat = this.mat, dark = this.dark;
    // Main receiver and barrel
    bx(0.085, 0.115, 0.6, 0, 0, 0.05, mat, g);
    cyl(0.024, 1.25, 0, 0.02, -0.92, dark, g); 
    
    // Aggressive Muzzle Brake
    bx(0.05, 0.05, 0.18, 0, 0.02, -1.6, dark, g);
    bx(0.06, 0.06, 0.03, 0, 0.02, -1.54, mat, g);
    bx(0.06, 0.06, 0.03, 0, 0.02, -1.6, mat, g);
    bx(0.06, 0.06, 0.03, 0, 0.02, -1.66, mat, g);

    // Magazine
    this.magMesh = bx(0.055, 0.16, 0.14, 0, -0.14, -0.06, mat, g); this.magY = -0.14;
    
    // Stock with adjustable cheek rest
    const stock = bx(0.075, 0.13, 0.44, 0, -0.02, 0.5, mat, g); stock.rotation.x = 0.04;
    bx(0.08, 0.04, 0.18, 0, 0.06, 0.46, dark, g).rotation.x = 0.04; // Cheek rest pad
    bx(0.07, 0.14, 0.05, 0, -0.02, 0.73, dark, g); // Buttpad
    
    // Grip
    bx(0.05, 0.14, 0.07, 0, -0.13, 0.2, mat, g).rotation.x = 0.3;

    // Detailed Scope
    cyl(0.052, 0.56, 0, 0.135, -0.1, mat, g); // Main tube
    cyl(0.066, 0.12, 0, 0.135, -0.36, mat, g); // Front bell (sunshade)
    cyl(0.062, 0.07, 0, 0.135, 0.14, mat, g); // Eyepiece
    
    // Scope Turrets (Windage/Elevation)
    cyl(0.03, 0.05, 0, 0.17, -0.1, dark, g, 'y'); // Elevation (top)
    cyl(0.03, 0.05, 0.06, 0.135, -0.1, dark, g, 'x'); // Windage (right)
    cyl(0.03, 0.05, -0.06, 0.135, -0.1, dark, g, 'x'); // Parallax (left)
